...
//    @KafkaListener(topics = "${setting.topics}",groupId = "${spring.kafka.consumer.group-id}")
    // 그룹은 Kafka의 소비자(Consumer)들이 함께 동작하는 논리적 집합을 나타냅니다. 소비자 그룹 내의 각 소비자는 특정 토픽의 파티션에서 메시지를 읽어옵니다.
    @KafkaListener(topics = "oingdaddy", groupId = "group-id-a")
...
//            , Acknowledgment acknowledgment // 수동 커밋 관리. 를 하므로 Acknowledgment 관련 에러가 발생한다. 그래서 이를 수동으로 하는 로직을 짜줘야함.
    ...
    @KafkaListener(topics = "quiz", groupId = "group-id-a")
    public void consumeQuiz(ConsumerRecord<String, String> consumerRecord, @Header(KafkaHeaders.OFFSET) Long offset
//            , Acknowledgment acknowledgment // 수동 커밋 관리. 를 하므로 Acknowledgment 관련 에러가 발생한다. 그래서 이를 수동으로 하는 로직을 짜줘야함.
            , Consumer<?, ?> consumer){
        try {
            /**
